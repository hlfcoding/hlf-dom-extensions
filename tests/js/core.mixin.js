...
(function() {
  ...
  define(['jquery', 'underscore', 'hlf/jquery.extension.hlf.core'], function($, _, hlf) {
    ...
    QUnit.test('createMixin', function(assert) {
      ...
      result = hlf.createMixin(this.mixins, this.mixinName, this.mixin);
      assert.strictEqual(result, false, 'Mixin should not have been re-added to mixin collection.');
    });
    assertMixinInstance = function(assert) {
      ...
      assert.strictEqual(this.instance.someOtherProperty, 'baz', 'Mixin once method should have invoked properly.');
    };
    ...
    assertDynamicMixinInstance = function(assert) {
      ...
      assert.strictEqual(this.instance.someMethod(), this.dependencies.valueA, 'Dynamic mixin method should have been generated properly.');
    };
